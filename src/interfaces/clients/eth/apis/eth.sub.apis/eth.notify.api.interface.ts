...
export interface IEthNotifyApi extends IConfigurable<IServerConfig> {
	subscribePushNotifications(token: string, addresses: string[], types: string[]): Promise<EthSubscribeToken>;
	unsubscribePushNotifications(token: string, addresses: string[], types: string[]): Promise<boolean>;
}
